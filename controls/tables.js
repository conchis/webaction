 ...
 new function(_) {
    ...
    var Table = Control.extend({
    	...
    	measure: function () {
            ...
            var width = 0;
            for (var index = 0; index < columns.length; index += 1)
                    width += columns[index].width + BORDER_WIDTH;
            var inner_width = width;
            if (jQuery.browser.mozilla) 
                inner_width += 2*MOZILLA_PADDING;
            this.inner_width = inner_width;
            this.width = inner_width + SCROLL_BORDER;
    	},
    ...
    var TableHeader = Control.extend({
    	...
    	generate: function (dom_element) {
			...
			dom_element.append(table({
					id: table_id, style: {width: this.inner_width}},
				tr()));
    	...
    	addColumn: function (index, column, row_element) {
    		...
    		var button = new ColumnButton({model: column, width: width});
    ...
    var TableBody = Control.extend({
    	...
    	generate: function (dom_element) {
            ...
            dom_element.append(table({style:{width: this.inner_width}}));
