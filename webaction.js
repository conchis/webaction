...
return;var table=jQuery("#"+this.id+" table");this.table=table;table.find("tr").remove();var self=this;model.each(function(item,index){var row_id=self.id+"_"+index;self.appendRow(table,row_id,item,index);self.bindRow(row_id,index,item);});this.rows.length=model.size();},appendRow:function(table,row_id,item,index){var model=this.model;var columns=this.columns;var cells=new Array();var rows=this.rows;var row_items=this.row_items;for(var column_index=0;column_index<columns.length;column_index+=1)
...
new function(_){var scroll_bar=new base2.Package(this,{name:"scroll_bar",version:"0.1",imports:"controls,options,generator",exports:"ScrollBar"});eval(this.imports);var ScrollBar=Control.extend({class_name:"ScrollBar",BUTTON_HEIGHT:14,constructor:function(options){this.base(options,{css_class:'wa_sbar'});this.value=this.option("value",0);this.max=this.option("max",100);this.shown=this.option("shown",-1);this.slide_top=this.BUTTON_HEIGHT;this.slide_height=this.height-2*this.BUTTON_HEIGHT;this.is_tracking=false;this.start_y=0;this.start_value=0;},generate:function(dom_element){var up_button=new ScrollButton({css_class:"wa_sbar_up",pressed_css:"wa_sbar_up_pressed"});this.add(up_button);up_button.addListener("up",this,"onUpPressed");this.up_button=up_button;var handle=new Handle();this.add(handle);this.handle=handle;var down_button=new ScrollButton({css_class:"wa_sbar_down",pressed_css:"wa_sbar_down_pressed",top:this.height-this.BUTTON_HEIGHT-1});this.add(down_button);down_button.addListener("up",this,"onDownPressed");this.down_button=down_button;this.connectEvents(dom_element);this.update();},connectEvents:function(dom_element){var self=this;dom_element.mousedown(function(event){self.startTracking(event);});var doc_element=jQuery(document);doc_element.mousemove(function(event){if(self.is_tracking)self.track(event);});doc_element.mouseup(function(event){self.stopTracking(event);});},update:function(){var slide_top=this.BUTTON_HEIGHT;var slide_height=this.slide_height;var handle_top=slide_top+Math.round(slide_height*this.value/this.max);var shown=this.shown;if(shown<1)
shown=Math.round(this.max/10);var handle_height=Math.round(slide_height*shown/this.max);this.handle.adjust(handle_top,handle_height,this.is_tracking);},setScroll:function(value,shown){if(shown==undefined)
shown=this.shown;this.value=Math.round(value);this.shown=Math.round(shown);this.update();},setMax:function(max){this.max=max;if(this.value>max)
this.value=max;this.update();},onUpPressed:function(){if(this.value>0){this.value-=1;this.update();}},onDownPressed:function(){if(this.value<(this.max-this.shown)){this.value+=1;this.update();}},startTracking:function(event){this.is_tracking=true;this.start_y=event.clientY;this.start_value=this.value;},track:function(event){var offset=event.clientY-this.start_y;var scaled_offset=Math.round(this.max*offset/this.slide_height);var shown=this.shown;if(shown<1){shown=Math.round(this.max/10);this.shown=shown;}
var new_value=Math.min(Math.max(0,this.start_value+scaled_offset),this.max-shown);if(new_value!=this.value){this.value=new_value;this.update();}},stopTracking:function(event){this.is_tracking=false;this.update();}});var ScrollButton=Control.extend({class_name:"ScrollButton",constructor:function(options){this.base(options,{css_class:"wa_sbar_button"});this.pressed_css=this.option("pressed_css","wa_sbar_button_pressed");},generate:function(dom_element){var self=this;dom_element.mousedown(function(){self.onMouseDown();});dom_element.mouseup(function(){self.onMouseUp();});dom_element.mouseleave(function(){self.onMouseLeave();})},onMouseDown:function(){this.dom_element.addClass(this.pressed_css);this.broadcast("down");},onMouseUp:function(){this.dom_element.removeClass(this.pressed_css);this.broadcast("up");},onMouseLeave:function(){this.dom_element.removeClass(this.pressed_css);}});var Handle=Control.extend({class_name:"Handle",constructor:function(options){this.base(options,{css_class:'wa_sbar_handle',top:ScrollBar.BUTTON_HEIGHT});},generate:function(dom_element){},adjust:function(top,height,is_tracking){this.dom_element.css("top",top);this.dom_element.css("height",height-2);if(is_tracking)
this.dom_element.addClass("wa_sbar_handle_pressed");else
this.dom_element.removeClass("wa_sbar_handle_pressed");}});eval(this.exports);};