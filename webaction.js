...
// core/generator.js
new function(_){var generator=new base2.Package(this,{name:"generator",version:"0.1",imports:"",exports:"element,tag,div,span,ul,ol,li,table,tr,th,td,p,a,br,form,"
...
return;this.is_ascending=is_ascending;this.broadcast("changed");},compareItems:function(item_a,item_b){if(!(this.sortable&&this.is_sorted))
...
result=-result;return result;},render:function(item,index){var attributes={style:'width:'+this.width+'px'};if(this.css_class!=null)
attributes['class']=this.css_class;return td(attributes,this.getValue(item));}});Column.implement(OptionsMixin);var Table=Control.extend({class_name:"Table",constructor:function(options){this.base(options);this.initializeColumns(this.option("columns",[]));this.filter=null;var model=this.option("model",null);if(model!=null)
...
width+=columns[index].width+BORDER_WIDTH;var inner_width=width;if(jQuery.browser.mozilla)
inner_width+=2*MOZILLA_PADDING;this.inner_width=inner_width;this.width=inner_width+SCROLL_BORDER;},setModel:function(model){this.model=model;this.update();},generate:function(dom_element){var table_header=new TableHeader(this.columns,{width:this.width,inner_width:this.inner_width});table_header.makeControl(dom_element);var table_body=new TableBody(this.filter,this.columns,{width:this.width,inner_width:this.inner_width,height:this.height-TOP_BAR_HEIGHT});table_body.makeControl(dom_element);table_body.addListener("select",this);this.table_body=table_body;},update:function(){var model=this.model;if(model!=null&&this.filter==null&&typeof(this.table_body)!="undefined"){this.filter=new ListFilter(model);this.table_body.setModel(this.filter);}},select:function(index,item){this.broadcast("select",index,item);},getSelectedIndex:function(){return this.table_body.getSelectedIndex();},getSelectedItem:function(){return this.table_body.getSelectedItem();}});var TableHeader=Control.extend({class_name:"TableHeader",constructor:function(columns,options){this.base(options,{css_class:"wa_table_header"});this.inner_width=this.option('inner_width');this.columns=columns;},generate:function(dom_element){var table_id=this.id+"_table";dom_element.append(table({id:table_id,style:{width:this.inner_width}},tr()));var row_element=jQuery("#"+table_id+" tr");var columns=this.columns;for(var index=0;index<columns.length;index+=1)
this.addColumn(index,columns[index],row_element);},addColumn:function(index,column,row_element){column.addListener("changed",this);var width=column.width;var button=new ColumnButton({model:column,width:width});button.makeControl(row_element);},changed:function(source){if(source.is_sorted){var columns=this.columns;for(var index=0;index<columns.length;index+=1){var column=columns[index];column.setSorted(column===source);}}}});var ColumnButton=Control.extend({class_name:"ColumnButton",constructor:function(options){var model=options["model"];this.base(options,{css_class:"wa_table_header_cell",width:model.width});this.setModel(model);},setModel:function(model){this.model=model;var self=this;model.addListener("changed",this,"update");this.resetTooltip();},element_tag:'th',generate:function(dom_element){var model=this.model;dom_element.append(span({'class':'wa_sort_text'},model.label));dom_element.append(span({}," "));var self=this;dom_element.click(function(){self.toggle();});this.resetTooltip();},resetTooltip:function(){var model=this.model;var dom_element=this.dom_element;if(model==null||dom_element==null)
return;if(model.sortable)
dom_element.attr('title',"Click to sort column");else
dom_element.attr('title',null);},toggle:function(){var model=this.model;if(model.is_sorted)
...
else{dom_element.removeClass("wa_table_cell_highlight");icon.removeClass("wa_sort_icon_down");icon.removeClass("wa_sort_icon_up");}}});var TableBody=Control.extend({class_name:"TableBody",constructor:function(model,columns,options){this.base(options,{css_class:'wa_table_body',height:200});this.inner_width=this.option('inner_width');this.initializeColumns(columns);this.selected=-1;this.rows=[];this.row_items=[];if(model!=null)
...
this.selected=selected;var row_id=this.id+"_"+selected;var row=jQuery("#"+row_id+" > td",this.dom_element);row.addClass("wa_table_selected");},generate:function(dom_element){dom_element.css({height:this.height,width:this.width});dom_element.append(table({style:{width:this.inner_width}}));this.generateRows(dom_element);},generateRows:function(dom_element){var model=this.model;if(this.model==null)
return;var table=jQuery("#"+this.id+" table");this.table=table;table.empty();var self=this;model.each(function(item,index){var row_id=self.id+"_"+index;self.appendRow(table,row_id,item,index);self.bindRow(row_id,index,item);});this.rows.length=model.size();},appendRow:function(table,row_id,item,index){var model=this.model;var columns=this.columns;var cells=new Array();var rows=this.rows;var row_items=this.row_items;for(var column_index=0;column_index<columns.length;column_index+=1)
